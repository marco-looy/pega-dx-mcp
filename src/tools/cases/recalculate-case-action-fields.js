...
export class RecalculateCaseActionFieldsTool extends BaseTool {
  ...
  formatSuccessResponse(caseID, actionID, data, eTag, options) {
    ...
    if (data.data) {
      ...
      if (content && Object.keys(content).length > 0) {
        ...
        response += '- **Field Visibility**: Only visible fields in case action view used in calculations\n';
      }
    ...
    response += '- **Field Scope**: Only fields present in the case action view can be calculated\n';
    response += '- **Context Requirements**: Field and when contexts must be valid for the current case action\n';
  ...
  formatErrorResponse(caseID, actionID, error, options) {
    const { eTag, calculations, content, pageInstructions, originChannel } = options;
    
    let response = `## Error recalculating case action fields: ${actionID}\n\n`;
    ...
    switch (error.type) {
      case 'NOT_FOUND':
        response += '\n**Suggestion**: Verify both the case ID and action ID are correct. The case might not exist, might be closed, or the specific action might not be available for this case. Also verify that the specified fields and when conditions exist and are accessible within the case action context.\n';
        break;
      case 'FORBIDDEN':
        response += '\n**Suggestion**: Check if you have the necessary permissions to recalculate fields for this case action. The case might be restricted to specific users or roles, or the fields/whens requested might not be accessible to your user context.\n';
        break;
      case 'UNAUTHORIZED':
        response += '\n**Suggestion**: Authentication may have expired. The system will attempt to refresh the token on the next request.\n';
        break;
      case 'BAD_REQUEST':
        response += '\n**Suggestion**: Check the case ID format, action ID, and calculations structure. Case IDs should follow the pattern: {OrgID}-{AppName}-{CaseType} {CaseNumber}. Verify that:\n';
        ...
        response += '- Context values are valid for the case action scope\n';
        break;
      case 'VALIDATION_FAIL':
        ...
        response += '- Field or when references that don\'t exist in the case action view\n';
      ...
      case 'CONFLICT':
        ...
        response += '- Case state changes during calculation processing\n';
        response += '- Concurrent field updates from other operations\n';
        response += 'Retrieve the latest case action details to get the current eTag value before attempting recalculation.\n';
        break;
      case 'PRECONDITION_FAILED':
        response += '\n**Suggestion**: The eTag value provided does not match the current case state. Retrieve the latest case action details to get the current eTag value before attempting the recalculation operation.\n';
        break;
    ...
    if (error.type === 'VALIDATION_FAIL' || error.type === 'BAD_REQUEST' || error.type === 'UNPROCESSABLE_ENTITY') {
      ...
      response += '- Only fields and whens accessible in the case action view can be calculated\n';
      response += '- Field and when names must correspond to actual property and rule definitions\n';
      response += '- Context values must be valid page references for the current case scope\n';
